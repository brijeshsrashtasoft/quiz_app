...
class FirestoreConfig {
  ...
  static FirebaseFirestore _configureFirestore() {
    ...
    try {
      ...
      AppLogger.firebase('Firestore', 'Firestore configured with persistence enabled');
      
      if (kDebugMode) {
        // Enable additional logging in debug mode
        FirebaseFirestore.setLoggingEnabled(true);
        AppLogger.firebase('Firestore', 'Firestore logging enabled for debug mode');
      }
  ...
  static DocumentReference<Map<String, dynamic>> getDocument(String collectionName, String documentId) {
    try {
      final document = instance.collection(collectionName).doc(documentId);
      AppLogger.firebase('Firestore', 'Accessing document: $collectionName/$documentId');
      return document;
    } catch (e) {
      AppLogger.error('Failed to access document: $collectionName/$documentId', e);
      throw FirestoreException(
  ...
  static Future<void> enableOfflinePersistence() async {
    try {
      ...
      AppLogger.firebase('Firestore', 'Offline persistence already configured in settings');
    } catch (e) {
