...
class GameSessionRealtimeService {
  ...
  Stream<int> monitorCurrentQuestion(String sessionId) {
    return monitorGameSession(sessionId)
        ...
        .debounceTime(const Duration(milliseconds: 200)); // Debounce rapid changes
  }
  ...
  Future<void> _executeBatchUpdate() async {
    ...
    final updates = Map<String, List<Map<String, dynamic>>>.from(_pendingUpdates);
  ...
  Future<void> prefetchSession(String sessionId) async {
    try {
      final doc = await FirestoreConfig.getDocument('game_sessions', sessionId)
          .get(const GetOptions(source: Source.cache));
      
      if (!doc.exists) {
        // Fetch from server if not in cache
        await FirestoreConfig.getDocument('game_sessions', sessionId)
            .get(const GetOptions(source: Source.server));
      }
      
      AppLogger.firebase(
        'RealtimeService',
        'Prefetched session $sessionId',
      );
    } catch (e) {
