...
class PinLookupService {
  ...
  Future<Result<String?>> lookupSessionByPin(String pin) async {
    ...
    try {
      ...
      return Result.failure(
        ServerFailure(
          message: 'Failed to look up PIN: ${e.toString()}',
        ),
      );
    }
  }
    ...
    try {
      ...
      if (pinsToQuery.isNotEmpty) {
        ...
        for (final chunk in chunks) {
          ...
          final foundPins = query.docs.map((doc) => doc.data()['pin'] as String).toSet();
      ...
      return Result.failure(
        ServerFailure(
          message: 'Failed to validate PINs: ${e.toString()}',
        ),
      );
    }
  }
  ...
  void clearExpiredCache() {
    ...
    _lookupHistory.removeWhere((time) => 
  ...
  bool _checkRateLimit() {
    final now = DateTime.now();
    final recentLookups = _lookupHistory.where((time) => 
      now.difference(time).inMinutes < 1
    ).length;
